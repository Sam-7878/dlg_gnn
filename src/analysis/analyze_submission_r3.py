"""Statistical/calibration/risk analysis of the frozen R3 predictions."""
from pathlib import Path
import json
import numpy as np
import pandas as pd
from scipy.special import expit
from analysis.submission_r3_statistics import (METRICS,paired_bootstrap,mcnemar,holm,
    calibration_metrics,reliability,exact_upper)
from gog_fraud.production.submission_r3 import (read_config,save_json,calibrate,logits,best_threshold,metrics)
from evidence.experiment_identity import digest


def main():
    cfg=read_config(); root=Path(cfg['output_root'])
    for sub in ('statistics','calibration','risk'): (root/sub).mkdir(parents=True,exist_ok=True)
    boot=[]; mc=[]; calibration=[]; bins=[]; risks=[]; paired_samples=[]
    for seed in cfg['seeds']:
        source=root/'offline'/f'seed{seed}'; selection=json.loads((source/'selection.json').read_text())
        frames={s:pd.read_csv(source/f'{s}_predictions.csv') for s in ('validation','test')}
        test=frames['test']; y=test.label.to_numpy()
        result,samples=paired_bootstrap(y,test.direct_only_score.to_numpy(),test.primary_score.to_numpy(),
        ...
        for metric,values in result.items(): boot.append({'seed':seed,'metric':metric,**values})
        mc.append({'seed':seed,**mcnemar(y,test.direct_only_prediction.to_numpy(),test.primary_prediction.to_numpy())})
        for split,frame in frames.items():
            for name,p in [('Raw Level-1 GIN',frame.raw_fast.to_numpy()),('Calibrated Level-1 GIN',frame.direct_only_score.to_numpy()),('Final selective score',frame.primary_score.to_numpy())]:
                calibration.append({'seed':seed,'split':split,'score_stage':name,**calibration_metrics(frame.label,p)})
                for row in reliability(frame.label,p,10,True): bins.append({'seed':seed,'split':split,'score_stage':name,**row})
        valid=frames['validation']; fv=valid.direct_only_score.to_numpy(); ft=test.direct_only_score.to_numpy()
        dv=calibrate(valid.raw_deep.to_numpy(),selection['deep_map']); dt=calibrate(test.raw_deep.to_numpy(),selection['deep_map'])
        w=selection['fast_weight']; fusedv=expit(w*logits(fv)+(1-w)*logits(dv)); fusedt=expit(w*logits(ft)+(1-w)*logits(dt))
        for budget in cfg['risk']['budgets']:
            ...
            for split,frame,fast,fused,route in [('validation',valid,fv,fusedv,routev),('test',test,ft,fusedt,routet)]:
                score=np.where(route,fused,fast); pred=score>=threshold; yy=frame.label.to_numpy()
                for scope in ('pooled','ethereum','bsc','polygon'):
                    ...
                    direct=mask&~route; support=int((direct&(yy==1)).sum()); missed=int((direct&(yy==1)&~pred).sum())
                    risks.append({'seed':seed,'split':split,'chain_scope':scope,'requested_budget':budget,
    ...
    for row,adjusted in zip(mc,holm([r['p_value'] for r in mc])): row['adjusted_p_value']=float(adjusted)
    ...
    save_json(root/'statistics/production_paired_bootstrap.json',{'rows':boot,'resamples':cfg['statistics']['resamples'],
    ...
    # Same contracts across seeds: joint row indices preserve cross-seed dependence.
    # We deliberately do not present an independent-per-seed resampling as a hierarchical CI.
    aggregate=[]
    for metric in METRICS:
        values=np.array([r['delta'] for r in boot if r['metric']==metric])
        aggregate.append({'metric':metric,'mean_delta':float(values.mean()),'median_delta':float(np.median(values)),
    ...
    save_json(root/'statistics/claim_status.json',{'track':'B','predeclared':True,'aggregate':aggregate,
        'confirmatory_claim_allowed':False,'hierarchical_bootstrap':'not estimated: same test contracts across five models; conditional per-model CIs only'})
