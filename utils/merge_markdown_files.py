...
def merge_markdown_files(base_dir, output_filepath):
    ...
    target_prefixes = [f"{i:02d}-" for i in range(30, 36)]  # 30부터 35까지로 수정
    
    # 병합할 문서들을 열기 (덮어쓰기 모드)
    with open(output_filepath, 'w', encoding='utf-8') as outfile:
        outfile.write("# 30 ~ 35 병합된 작업 보고서\n\n")
        
        # 디렉토리 내의 항목들을 알파벳/숫자 순으로 정렬하여 탐색
        for folder_name in sorted(os.listdir(base_dir)):
            folder_path = os.path.join(base_dir, folder_name)
            
            # 항목이 디렉토리인지, 그리고 타겟 접두사(30~35)로 시작하는지 확인
...
if __name__ == "__main__":
   ...
   # OUTPUT_FILE = "Merged_Reports_00_to_10.md"
    OUTPUT_FILE = "./dlg_gnn/docs/work_reports/Merged_Reports_30_to_35.md"
