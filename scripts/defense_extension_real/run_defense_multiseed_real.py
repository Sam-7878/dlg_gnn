#!/usr/bin/env python3
"""
Multi-Seed Benchmark Runner for Defense Extension (Real Rebuild D3).
Executes 8 models x 5 seeds across DARPA-TC-THEIA and LANL-RedTeam.
Applies transductive train/val/test splits, validation-selected thresholds,
and full process-level memory isolation.
"""
...
from gog_fraud.extensions.defense.defense_registry import (
    DEFENSE_DATASETS,
...
def run_defense_cell(dataset_name: str, model_name: str, seed: int, config: dict, output_dir: Path, data_dir: Path) -> dict:
    ...
    if raw_path.exists():
        try:
            ...
            if cached.get("status") in {"success", "failed_oom", "unsupported_resource_exact_implementation"}:
    ...
    try:
        data = load_defense_dataset(dataset_name, base_dir=data_dir).clone()
        ...
        # Numerically stable feature normalization (leakage-free train-set scaling)
        if data.x is not None and data.x.numel() > 0:
            x_float = data.x.float()
            max_vals = x_float.max(dim=0).values
            large_cols = max_vals > 10.0
            if large_cols.any():
                x_float[:, large_cols] = torch.log1p(torch.clamp(x_float[:, large_cols], min=0.0))
            train_t = torch.as_tensor(train_idx, dtype=torch.long)
            mean = x_float[train_t].mean(dim=0, keepdim=True)
            std = x_float[train_t].std(dim=0, keepdim=True)
            std[std == 0.0] = 1.0
            std[torch.isnan(std)] = 1.0
            data.x = (x_float - mean) / std
        ...
        record = {
            ...
            "benchmark_origin": "defense_real_rebuild",
        ...
        err_msg = str(e)
        is_oom = "out of memory" in err_msg.lower() or "cuda" in err_msg.lower() or "alloc" in err_msg.lower()
        status_name = "failed_oom" if is_oom else "failed_other"
        log.error("Cell Resource Limit/Error: %s/%s/seed%d -> %s", dataset_name, model_name, seed, err_msg)
        record = {
            "run_id": str(uuid.uuid4()),
            "benchmark_origin": "defense_real_rebuild",
            "dataset": dataset_name, "model": model_name, "seed": int(seed),
            "status": status_name, "support_status": "unsupported_resource_exact_implementation",
            "error": err_msg,
        }
        raw_path.write_text(json.dumps(record, indent=2) + "\n", encoding="utf-8")
        return record
...
def main():
    parser = argparse.ArgumentParser(description="Run Defense Extension Multi-Seed Benchmark (Real D3).")
    parser.add_argument("--config", type=str, default="configs/benchmark/sci_defense_extension.yaml")
    parser.add_argument("--data-dir", type=str, default="outputs/sci_defense_extension_real/graphs")
    parser.add_argument("--output-dir", type=str, default="outputs/sci_defense_extension_real/benchmark")
    ...
    parser.add_argument("--single-cell", action="store_true", help="Execute only one specified cell.")
    ...
    # Single cell execution (used by subprocess isolation)
    if args.single_cell:
        assert args.dataset and args.model and args.seed is not None, "Single cell requires --dataset, --model, --seed"
        rec = run_defense_cell(args.dataset, args.model, args.seed, cfg, output_root, data_root)
        print(f"CELL_RESULT_STATUS: {rec.get('status')}")
        return

    # Master loop with subprocess process isolation per cell
    raw_dir = output_root / "raw"
    raw_dir.mkdir(parents=True, exist_ok=True)

    log.info("Starting Master Benchmark Runner across %s datasets, %s models, %s seeds",
             args.datasets, args.models, args.seeds)

    for d in args.datasets:
        for m in args.models:
            for s in args.seeds:
                cell_json = raw_dir / f"{d}__{m}__seed{s}.json"
                if cell_json.exists():
                    try:
                        cached = json.loads(cell_json.read_text(encoding="utf-8"))
                        if cached.get("status") in {"success", "failed_oom", "unsupported_resource_exact_implementation"}:
                            log.info("Skipping already completed cell: %s", cell_json.name)
                            continue
                    except Exception:
                        pass
                ...
                log.info(">>> Launching isolated cell: %s / %s / seed%d", d, m, s)
                sub_res = subprocess.run(cmd, capture_output=False)
                if sub_res.returncode != 0:
                    log.warning("Cell process returned code %d for %s/%s/seed%d", sub_res.returncode, d, m, s)

    # Build master benchmark_raw.csv
    import pandas as pd
    all_json_files = sorted(raw_dir.glob("*.json"))
    records = []
    ...
    if records:
        ...
        raw_csv = output_root / "benchmark_raw.csv"
        df.to_csv(raw_csv, index=False)
        raw_sha = hashlib.sha256(raw_csv.read_bytes()).hexdigest()
        (output_root / "benchmark_raw.csv.sha256").write_text(raw_sha + "\n", encoding="utf-8")
        log.info("Master Defense benchmark_raw.csv saved: %d rows (SHA-256: %s)", len(df), raw_sha)

    print("Defense Extension Multi-Seed Benchmark Completed Successfully.")
