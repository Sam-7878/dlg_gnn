#!/usr/bin/env python3
"""
test_mdpi_bundle_clean_compile_after_preprint_update.py

Round P1 Freeze Gate: Verifies that the MDPI Applied Sciences submission bundle
(publication/mdpi/DLG_Benchmark_MDPI_Submission.zip) contains all required assets
including cover_letter.md and Definitions/, and compiles cleanly in an isolated
temporary directory to produce a publication-ready PDF.
"""

from pathlib import Path
import shutil
import subprocess
import tempfile
import zipfile
import pytest

REPO_ROOT = Path(__file__).resolve().parents[3]
MDPI_DIR = REPO_ROOT / "publication" / "mdpi"
MDPI_ZIP = MDPI_DIR / "DLG_Benchmark_MDPI_Submission.zip"
MDPI_PDF = MDPI_DIR / "DLG-Benchmark.pdf"
COVER_LETTER = MDPI_DIR / "cover_letter.md"


def test_mdpi_bundle_files_and_cover_letter():
    assert MDPI_ZIP.exists(), f"Missing {MDPI_ZIP}"
    assert MDPI_PDF.exists(), f"Missing {MDPI_PDF}"
    assert COVER_LETTER.exists(), f"Missing {COVER_LETTER}"

    with zipfile.ZipFile(MDPI_ZIP, "r") as zf:
        names = zf.namelist()
        assert "DLG-Benchmark.tex" in names
        assert "references.bib" in names
        assert "cover_letter.md" in names
        assert any("Definitions/mdpi.cls" in n for n in names)
        assert any(n.startswith("generated/") for n in names)


def test_mdpi_pdf_artifact_valid():
    assert MDPI_PDF.stat().st_size > 200 * 1024, "MDPI PDF too small"


def _run_tex_cmd(cmd_list, cwd: Path):
    if shutil.which("pdflatex"):
        res = subprocess.run(cmd_list, cwd=cwd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
        return res
    # Fallback to WSL
    wsl_exe = shutil.which("wsl")
    if wsl_exe:
        wsl_path = str(cwd).replace("\\", "/")
        if ":" in wsl_path:
            drive, rest = wsl_path.split(":", 1)
            wsl_path = f"/mnt/{drive.lower()}{rest}"
        cmd_str = f"cd '{wsl_path}' && {' '.join(cmd_list)}"
        res = subprocess.run([wsl_exe, "-d", "Ubuntu", "--", "bash", "-c", cmd_str],
                             stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
        return res
    pytest.skip("Neither pdflatex nor WSL is available for isolated compilation test")


def test_isolated_clean_compilation():
    with tempfile.TemporaryDirectory() as tmpdir:
        tmp_path = Path(tmpdir)
        with zipfile.ZipFile(MDPI_ZIP, "r") as zf:
            zf.extractall(tmp_path)

        # 4-pass compilation
        p1 = _run_tex_cmd(["pdflatex", "-interaction=nonstopmode", "-halt-on-error", "DLG-Benchmark.tex"], cwd=tmp_path)
        assert p1.returncode == 0, f"Pass 1 failed:\n{p1.stdout[-1500:]}"

        b = _run_tex_cmd(["bibtex", "DLG-Benchmark"], cwd=tmp_path)
        assert b.returncode == 0, f"BibTeX failed:\n{b.stdout[-1500:]}"

        p2 = _run_tex_cmd(["pdflatex", "-interaction=nonstopmode", "-halt-on-error", "DLG-Benchmark.tex"], cwd=tmp_path)
        assert p2.returncode == 0, f"Pass 2 failed:\n{p2.stdout[-1500:]}"

        p3 = _run_tex_cmd(["pdflatex", "-interaction=nonstopmode", "-halt-on-error", "DLG-Benchmark.tex"], cwd=tmp_path)
        assert p3.returncode == 0, f"Pass 3 failed:\n{p3.stdout[-1500:]}"

        out_pdf = tmp_path / "DLG-Benchmark.pdf"
        assert out_pdf.exists()
        assert out_pdf.stat().st_size > 200 * 1024
